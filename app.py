    ...
    try:
        ...
        if data:
            try:
                ...
                # Store in memory too
                _cache[key] = value
                _CACHE_EXPIRY = time.time() + 3600  # 1 hour
                return value
...
# Function to search and return player by name - directly from your baseline code
...
# Background task for player stats
@celery.task
def fetch_player_stats_in_background(player_name, is_top_player=False):
    ...
    try:
        # Simple direct approach like your baseline code
...
# Route for player stats - using simple approach like baseline code
...
def get_player_stats():
    ...
    try:
        # Direct fetch like baseline code
        career = PlayerCareerStats(player_id=player['id'])
        data = career.get_dict()
        result_set = data['resultSets'][0]
        headers = result_set['headers']
        ...
        # Process stats exactly like baseline
        stats = [dict(zip(headers, row)) for row in rows]
...
def get_last_5_games():
    ...
    try:
        # Direct approach but with PlayerGameLogs instead of playergamelogs
...
def get_top_players_stats():
    ...
    # Using direct approach like baseline code
    top_players_stats = []
    ...
    for player_name in top_players:
        ...
        try:
            ...
            # Cache all stats
            set_to_cache(player_cache_key, stats, expiration=86400)  # 24 hours
